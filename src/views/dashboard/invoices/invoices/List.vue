...
<script>
  ...
  export default {
    ...
    data: (vm) => ({
      ...
      headers: [
        {
          text: vm.$t('invoices.id'),
          align: 'start',
          sortable: false,
          value: 'id',
        },
        { text: vm.$t('invoices.date'), sortable: false, value: 'receipt_date' },
        { text: vm.$t('invoices.invoiceNumber'), sortable: false, value: 'receipt_number' },
        { text: vm.$t('invoices.clientName'), sortable: false, value: 'client.name' },
        { text: vm.$t('invoices.invoiceTotal'), sortable: false, value: 'total_egp' },
        { text: vm.$t('invoices.paymentStatus'), sortable: false, value: 'paymentStatus' },
    ...
    methods: {
      async fetchAllItems () {
        ...
        const list = await invoicesService.getAllItems(itemsPerPage, page, pageNumber)
        this.list = list.data
      ...
      async deleteItem (itemDetails) {
        this.loading = true
        this.disabled = true
        const deleteItem = await invoicesService.deleteItem(itemDetails.id)
      ...
      async changeStatus (itemDetails) {
        this.loading = true
        this.disabled = true
        const changeStatus = await invoicesService.changeStatus(itemDetails)
